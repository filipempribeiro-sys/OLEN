...
function bindConversationRows(){$$("[data-conv]").forEach(b=>{let timer;b.onclick=()=>activate(b.dataset.conv);b.onpointerdown=e=>{if(e.pointerType!=="mouse")timer=setTimeout(()=>openLong(b.dataset.conv),500)};["pointerup","pointercancel","pointerleave"].forEach(n=>b.addEventListener(n,()=>clearTimeout(timer)))});$$("[data-conv-more]").forEach(b=>b.onclick=e=>{e.stopPropagation();openLong(b.dataset.convMore)})}
